...
function createPostHtml(postData, largeFont = false) {
    ...
    if(isRetweet) {
        retweetText = `<span>
                        ...
                        Retuiteado por <a href='/profile/${retweetedBy}'>@${retweetedBy}</a>    
                    </span>`
    }
...
function timeDifference(current, previous) {
    ...
    if (elapsed < msPerMinute) {
        if(elapsed/1000 < 30) return "En este momento";
...
function outputPosts(results, container) {
    ...
    if (results.length == 0) {
        container.append("<span class='noResults'>No hay publicaciones.</span>")
    }
}
...
function getNotificationText(notification) {
    ...
    if(notification.notificationType == "retweet") {
        text = `${userFromName} hizo retweet a una de tus publicaciones`;
    }
    else if(notification.notificationType == "postLike") {
        text = `${userFromName} le gusta tu publicacion`;
    }
    else if(notification.notificationType == "reply") {
        text = `${userFromName} respondió a una de tus publicaciones`;
    }
    else if(notification.notificationType == "follow") {
        text = `${userFromName} te sigue`;
    }
